#python3
## server side of SiFT login protocol
...
class SiFT_LOGIN:
    ...
    def parse_login_req(self, login_req):
        ...
        # Same structure as server or different?
    ...
    def handle_login_server(self):
        ...
        # add timestamp check here or in separate try statement?
        try:
        ...
        if not ((current_time - window_ns) <=  float(timestamp) <= (current_time + window_ns)):
        ...
        ikm = bytes.fromhex(login_req_struct['client_random']) + server_random
